    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex h-full flex-1 flex-col gap-6 rounded-xl p-6">
            <div class="flex items-center justify-between">
                <h1 class="text-2xl font-bold text-pink-400">Proficiency Tests</h1>
                <Link
                ...
                <table class="min-w-full table-auto text-left text-sm text-pink-400">
                    ...
                    <tbody class="divide-y divide-pink-100 bg-white">
                        <tr v-for="test in sortedTests" :key="test.id" class="hover:bg-pink-50">
                            <td class="flex items-center gap-2 px-4 py-2 text-[#01006c]">
                                <BookOpen class="h-4 w-4 text-[#01006c]" />
                                {{ test.title }}
                            </td>
                            <td class="px-4 py-2 text-[#01006c] capitalize">{{ test.type }}</td>
                            <td class="px-4 py-2 text-[#01006c]">{{ new Date(test.scheduled_at).toLocaleString() }}</td>
                            <td class="px-4 py-2 text-[#01006c]">{{ new Date(test.due_date).toLocaleString() }}</td>
                            <td class="px-4 py-2">{{ test.year_level.name }}</td>
                            <td class="px-4 py-2 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <Link :href="`/proficiency-test/${test.id}/edit`" class="text-blue-600 hover:text-blue-800">
                                        <Pencil class="h-5 w-5" />
                                    </Link>
                                    <Link :href="`/proficiency-test/${test.id}/questions/create`" class="text-pink-500 hover:text-pink-600">
                                        <PlusCircle class="h-5 w-5" />
                                    </Link>
                                    <button @click="confirmDelete(test.id)" class="text-red-600 hover:text-red-800">
