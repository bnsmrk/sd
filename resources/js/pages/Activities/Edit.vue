...
const selectedModule = computed(() =>
  props.modules.find((m) => m.id === Number(form.module_id))
);
</script>
...
<template>
  ...
  <AppLayout>
    <div class="mx-auto max-w-4xl space-y-6 p-6">
      ...
      <form
        @submit.prevent="form.post(`/activities/${props.activity.id}`, { forceFormData: true })"
        class="space-y-6"
      >
        <!-- Row 1 -->
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label class="flex items-center gap-1 text-sm font-medium text-gray-700">
              <CalendarClock class="w-4 h-4" /> Scheduled At
            </label>
            <input
              type="datetime-local"
        ...
        <div class="flex justify-end">
          <button class="mt-4 rounded bg-blue-600 px-6 py-2 text-white hover:bg-blue-700">
            Update
          </button>
        </div>
      </form>
